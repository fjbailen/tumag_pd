...
#Function that returns the merit function
#using all possible wavelength correlations between Stokes I and Q, U and V
#By F.J. Bailen
def fitfunc1_all_combs(param, stokesin):
    D = param[0]
    theta = param[1]
    chi = param[2]
    ...
    #Computes the merit function
    out = minimize_for_model1_all_combs(iMM,stokesin)
...
#using all possible wavelength correlations between Stokes I and Q, U and V
#By F.J. Bailen
def minimize_for_model1_all_combs(iMM,bs):
    # apply a mueller matrix (rotation) to a 3D stokes vector 
    # (npixel,wavelength,4)
    new_stokes = np.einsum('ij,abj->abi',iMM, np.squeeze(bs))
...
def fit_mueller_matrix2(data,pthresh=0.02,norm=False,
                       region=[200,1200,200,1200],
                       last_wvl=None,plots=False):
    """
    fit_mueller_matrix but minimization correlations
    between from Stokes I to Q, U and V at ALL wavelengths
    """
    #Reorder axis to convert into dimensions: [x,y,wavelength,stokes]
    data=np.moveaxis(data,0,-1)
    data=np.moveaxis(data,0,-1)
    ...
    if plots is True:
        ...
        plt.close()        
    #Minimize merit function
    result = minimize(fitfunc1_all_combs, initial_guess, args=weak_region)
