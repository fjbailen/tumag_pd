...
Nseries=5 #Number of images recorded for the focused and defocused position
cam=1 #Cam 1 or 2
dir='../../Datos e2e/2024/Kiruna/2024-04-16'
dir_focus='RDoT/F517/CAM1'
dir_defocus='RDoT_PD/F517/CAM1'
dir_darks='DARKS/F517/CAM1' #Folder containing dark images
dir_flats='flats/F517/CAM1'
...
for i in range(Nseries):
    header,of0[:,:,i]=imr.Read_Image(dir+'/'+dir_focus+'/'+focus_path[i])
    #print(header)
    header1,od0[:,:,i]=imr.Read_Image(dir+'/'+dir_defocus+'/'+defocus_path[i])
    header2,darks_ima[:,:,i]=imr.Read_Image(dir+'/'+dir_darks+'/'+darks_path[i])
    header3,flats_ima[:,:,i]=imr.Read_Image(dir+'/'+dir_flats+'/'+flats_path[i])
...
#Dark and flat image
dark_average=np.mean(darks_ima,axis=2)
flat_average=np.mean(flats_ima,axis=2)
...
axs[0].imshow(of0[xmin:xmax,xmin:xmax,0],cmap='gray',vmin=0,vmax=1)
axs[1].imshow(od0[xmin:xmax,xmin:xmax,0],cmap='gray',vmin=0,vmax=1)
plt.show()
plt.close()
